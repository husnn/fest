...
contract('Token', async (accounts) => {
  ...
  describe('Supported interfaces', async () => {
    it('should support receiving ERC1155 tokens', async () => {
      const isERC1155Receiver = await TokenContract.supportsInterface('0xd9b67a26');
      assert.equal(isERC1155Receiver, true, 'Receiving ERC1155 is not supported');
    });
  
    it('should support secondary fees', async () => {
      const supportsSecondarySaleFees = await TokenContract.supportsInterface('0xb7799584');
      assert.equal(supportsSecondarySaleFees, true, 'Secondary sale fees are not supported');
    });
  ...
  describe('Access control', async () => {
    it('should have deployer as Default Admin', async () => {
      const result = await TokenContract.hasRole(DEFAULT_ADMIN_ROLE, accounts[0]);
    ...
    it('should have Default Admin as admin of role Admin', async () => {
      ...
      assert.equal(result, DEFAULT_ADMIN_ROLE, 'Default Admin is not admin of admins');
    });
    ...
    it('prevent granting Minter role to new account from non-admin', async () => {
      await expectRevert(
        TokenContract.grantRole(MINTER_ROLE, accounts[3], { from: accounts[2] }),
        'missing role'
      );
    });
  
    it('grant Minter role to new account from admin', async () => {
      const receipt = await TokenContract.grantRole(MINTER_ROLE, accounts[3], { from: accounts[0] });
  ...
  describe('Mint new token', async () => {
    ...
    it('prevent mint with different supply from signed hash', async () => {
      await expectRevert(
        TokenContract.mint(accounts[1], 6455, [], "", expiry, salt, signature, { from: accounts[2] }),
        'Invalid signature.'
      );
    });
    ...
    it('prevent mint with different salt from signed hash', async () => {
      await expectRevert(
        TokenContract.mint(accounts[1], supply, [], "", expiry, 544564, signature, { from: accounts[2] }),
        'Invalid signature.'
      );
    });
    ...
    it('prevent mint with different creator from signed hash', async () => {
      await expectRevert(
        TokenContract.mint(accounts[2], supply, [], "", expiry, salt, signature, { from: accounts[2] }),
        'Invalid signature.'
      );
    });
    ...
    it('execute mint from a non-minter, non-creator account', async () => {
      const receipt = await TokenContract.mint(
        accounts[1], supply, [], "", expiry, salt, signature, { from: accounts[2] }
      );
    ...
    it('prevent mint with same hash', async () => {
      await expectRevert(
        TokenContract.mint(accounts[1], supply, [], "", expiry, salt, signature, { from: accounts[2] }),
        'Approval exhausted.'
      );
    });
    ...
    it('prevent mint signed by non-minter', async () => {
      ...
      await expectRevert(
        TokenContract.mint(accounts[1], supply, [], "", expiry, 4321, signature, { from: accounts[2] }),
        'Invalid signature.'
      );
    });
    ...
    it('execute mint with invalid signature but sent from account with minter role', async () => {
      const receipt = await TokenContract.mint(
        accounts[1], supply, [], "", expiry, salt, signature, { from: accounts[0] }
      );
    ...
    it('prevent cancelling by non-minter and non-admin account', async () => {
      await expectRevert(
        TokenContract.revokeMintApproval(accounts[5], supply, expiry, salt, { from: accounts[6] }),
        'Permission denied.'
      );
    });
    ...
    it('cancel mint', async () => {
      ...
      const receipt = await TokenContract.revokeMintApproval(accounts[5], supply, expiry, salt, { from: accounts[3] });
      expectEvent(receipt, 'RevokeMint');

      await expectRevert(
        TokenContract.mint(accounts[5], supply, [], "", expiry, salt, signature, { from: accounts[5] }),
        'Approval exhausted.'
      );
    });
  });
  ...
  describe(('Mint with fees'), async () => {
    ...
    it('execute mint with fees', async () => {
      const receipt = await TokenContract.mint(accounts[1], 10, [
        {
  ...
  describe('Burn tokens', async () => {
    ...
    it('allow burning token from admin account', async () => {
      const receipt = await TokenContract.burn(accounts[1], 1, 1, { from: accounts[0] })
      expectEvent(receipt, 'TransferSingle');
    });

    it('allow burning token from creator account', async () => {
      const receipt = await TokenContract.burn(accounts[1], 1, 1, { from: accounts[1] })
      expectEvent(receipt, 'TransferSingle');
    });
  });
});